...
REM ============================================================
echo.
echo [*] Installing new driver from INF: "%DRIVER_INF_PATH%"

if not exist "%DRIVER_INF_PATH%" (
    echo [ERROR] INF file not found: "%DRIVER_INF_PATH%"
    echo        Make sure the INF file is in the same folder as this script.
    exit /b 2
)

"%PNPUTIL_EXE%" /add-driver "%DRIVER_INF_PATH%" /install
if errorlevel 1 (
    echo [ERROR] pnputil failed to install the driver. Please check the INF file and driver signature.
    exit /b 3
)

echo [OK] Driver installation command completed.

REM ============================================================
REM Step 5 - 加载过滤驱动
REM ============================================================
echo.
echo [*] Trying to load minifilter: "%FILTER_NAME%" ...

fltmc load "%FILTER_NAME%"
if errorlevel 1 (
    echo [WARN] Failed to load filter "%FILTER_NAME%" via "fltmc load".
    echo       - If this is a boot-start driver, please reboot and check again.
    echo       - Otherwise, please confirm the service name and INF configuration.
) else (
    echo [OK] Filter "%FILTER_NAME%" successfully loaded.
)

echo.
echo [*] Verifying if the minifilter is listed in "fltmc filters"...

fltmc filters | findstr /I "%FILTER_NAME%" >nul 2>&1
if errorlevel 1 (
    echo [WARN] Filter "%FILTER_NAME%" is not listed in "fltmc filters".
    echo       - If the driver is configured as boot-start, please reboot and check again.
    echo       - Otherwise, please check StartType and service configuration in the INF.
) else (
    echo [OK] Filter detected in filter list: "%FILTER_NAME%".
)

REM ============================================================
REM Step 6 - 将过滤器挂载到所有盘符卷
REM ============================================================
echo.
echo [*] Attaching filter "%FILTER_NAME%" to all local drive-letter volumes...

for /f "skip=1 tokens=1" %%V in ('fltmc volumes') do (
    set "vol=%%V"

    if /I "!vol:~1,1!"==":" (
        fltmc attach "%FILTER_NAME%" "!vol!" >nul 2>&1
        if errorlevel 1 (
            echo [WARN] Failed to attach filter "%FILTER_NAME%" to !vol!
        ) else (
            echo [OK] Filter "%FILTER_NAME%" attached to !vol!
        )
    ) else (
        echo [*] Skipping non-drive-letter volume: !vol!
    )
)
echo.
echo [DONE] Driver installation and load process completed.
endlocal
exit /b 0
